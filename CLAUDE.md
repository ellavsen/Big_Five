...
**Этап 2 закрыт целиком.** Полный список с verify-критериями — в
[docs/ROADMAP.md](docs/ROADMAP.md). Следующий — **Этап 3**: OCEAN как ядро оценки,
сворачивание мнимой мультиагентности, кросс-сессионная память.

## Закрытие осей

Ось закрывается **накоплением**, а не одним ярким сигналом. Считает `weigh_axis`
в [core/validity.py](core/validity.py): вес ведущего направления, число наблюдений,
набор источников, перевес над противоположным полюсом. Пороги — в `core/config.py`.

- Инвариант: **жёстко закрытая ось всегда закрыта и мягко.** До 2C было наоборот,
  и «мягкое» условие оказывалось строже «жёсткого». Закреплено тестом.
- Модули — слабый слой (`confidence` 0.25–0.3) и в одиночку ось не закрывают:
  их тексты постоянны, а `add_signals` режет дубли по `(axis, direction, text)`.
