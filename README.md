...
Mini App — отдельный процесс, общая с ботом только база:

```bash
uvicorn app.web:app --port 8000
```

Кнопка «Открыть карту» появится в боте, когда в `.env` будет `WEBAPP_URL` с адресом
на `https` — Telegram другие не открывает. Локально проще всего получить туннелем:
